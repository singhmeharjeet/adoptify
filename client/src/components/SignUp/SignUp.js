...
export default function SignUp() {
	...
	const validateFirstName = (event) => {
		...
		if (firstname == "") {
			...
			setErrFirstName(
				"First Name should be 3-16 characters long and shouldn't include any special character! "
			);
			setError(true);
		} else {
			setErrFirstName("");
	...
	const validateLastName = (event) => {
		...
		if (lastname == "") {
			...
			setErrLastName(
				"Last name should be 2-16 characters long and shouldn't include any special character! "
			);
			setError(true);
		} else {
			setErrLastName("");
	...
	const validateEmail = (event) => {
		...
		if (email == "") {
			...
			setErrEmail(
				"Invalid email address. Should be of format abc@abc.abc"
			);
			setError(true);
	...
	const validatePassword = (event) => {
		...
		if (password == "") {
			...
			setErrPassword("");
			setErrConfPassword(
				"Confirm password don't match with Password! Try again."
			);
			setError(true);
		} else {
			setErrPassword("");
						...
						<div className="signup-form-outline">
							...
							>
								<div>
									{isClicked ? (
										<div className="alert">
											<i className="circleErrorIcon">
												<FontAwesomeIcon
													icon={faExclamationCircle}
												></FontAwesomeIcon>
											</i>
											Please fill in the form with the
											appropriate values.{" "}
										</div>
									) : null}
								</div>
								...
								<div className="form-field">
									<label className="errMsg">{errEmail}</label>
								</div>
